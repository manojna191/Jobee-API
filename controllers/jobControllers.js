...
//Searh job with radius => /api/v1/jobs/:zipcode/:distance
exports.getJobsInRadius = catchAsyncErrors (async (req,res,next)=>{
    const {zipcode, distance} = req.params
    
    //getting latitude and langitutde from geocoder with zipcode
    const loc = await geoCoder.geocode(zipcode)
    const latitude = loc[0].latitude
    const longitude = loc[0].longitude

    const radius = distance / 3963 //3963 is radius of earth in miles
...
exports.jobStats = catchAsyncErrors (async (req,res,next)=>{
    ...
    if(stats.length == 0){
        return next(new ErrorHandler(`No stats found for the ${req.params.topic}`, 200))
    }
...
exports.applyJob = catchAsyncErrors(async (req,res,next)=>{
    ...
    //check if the last date of job has been passed or not
    if(job.lastDate < Date.now()){
        return next(new ErrorHandler('Application date has been passed', 400))
    }
